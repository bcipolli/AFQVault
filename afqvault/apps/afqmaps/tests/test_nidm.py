...
from afqvault.apps.afqmaps.nidm_results import NIDMUpload
from afqvault.apps.afqmaps.views import download_collection
...
class NIDMResultsTest(TestCase):
    ...
    def testParseNIDMZip(self):
        ...
        for name in self.files:
            ...
            afqmaps[name] = uploads[name].get_afqmaps()
        ...
        for name, info in self.files.items():
            first_map = sorted(afqmaps[name])[0]
            print first_map
            print info
            print name
            for field in 'name', 'type':
                self.assertEquals(first_map[field], info['output_row'][field])
            self.assertEquals(len(afqmaps[name]), info['num_afqmaps'])
    ...
    def testDownloadCollection_NIDM_results(self):
        ...
        image = save_statmap_form(image_path=os.path.join(TEST_PATH, 'test_data/afqmaps/all.nii.gz'),

...
def calculate_gene_expression_similarity(reduced_stat_map_data):
    ...
    with pd.HDFStore(store_file, 'r') as store:
        for donor_id in store.keys():
            ...
            expression_data.drop(
                expression_data.columns[na_mask], axis=1, inplace=True)
            ...
            regression_results = np.linalg.lstsq(
                np.c_[nifti_values, np.ones_like(nifti_values)], expression_data.T)
            results_df = pd.DataFrame(
                {"slope": regression_results[0][0]}, index=expression_data.index)
    ...
    group_results_df = pd.DataFrame({"t": t, "p": p}, columns=[
                                    't', 'p'], index=expression_data.index)
    ...
    probe_info = pd.read_csv("/ahba_data/probe_info_max1.csv",
                             index_col=0).drop(['chromosome', "gene_id"], axis=1)

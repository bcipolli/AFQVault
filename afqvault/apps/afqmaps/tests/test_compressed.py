...
from afqvault.apps.afqmaps.models import User, Collection, Image
...
class UploadFolderTestCase(TestCase):
    def setUp(self):
        ...
        with tarfile.open(os.path.join(self.tmpdir, 'example.tar.gz'), "w:gz") as tar:
            ...
            tar.add(os.path.join(test_path, 'test_data/afqmaps/beta_0001.nii.gz'))
            tar.add(os.path.join(test_path, 'test_data/afqmaps/motor_lips.nii.gz'))
            tar.add(os.path.join(test_path, 'test_data/afqmaps/WA3.nii.gz'))

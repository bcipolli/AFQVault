...
from afqvault.apps.afqmaps.models import Comparison, Similarity, User, Collection, Image
from afqvault.apps.afqmaps.tasks import save_voxelwise_pearson_similarity, get_images_by_ordered_id, save_resampled_transformation_single
from afqvault.apps.afqmaps.tests.utils import clearDB, save_statmap_form
from afqvault.apps.afqmaps.utils import split_4D_to_3D, get_similar_images
...
class ComparisonTestCase(TestCase):
    ...
    def setUp(self):
        ...
        image_nan = save_statmap_form(image_path=os.path.join(app_path,'test_data/afqmaps/motor_lips_nan.nii.gz'),
    ...
    def test_private_to_public_switch(self):
        ...
        app_path = os.path.abspath(os.path.dirname(__file__))
        private_image1 = save_statmap_form(image_path=os.path.join(app_path,'test_data/afqmaps/all.nii.gz'),
                                           collection=private_collection1,
                                           image_name = "image1")
        private_image2 = save_statmap_form(image_path=os.path.join(app_path,'test_data/afqmaps/motor_lips.nii.gz'),
    ...
    def test_add_DOI(self):
        ...
        app_path = os.path.abspath(os.path.dirname(__file__))
        image1 = save_statmap_form(image_path=os.path.join(app_path,'test_data/afqmaps/all.nii.gz'),
                                   collection=collection1,
                                   image_name = "image1")
        image2 = save_statmap_form(image_path=os.path.join(app_path,'test_data/afqmaps/motor_lips.nii.gz'),
    ...
    def test_get_similar_images(self):
        ...
        image1 = save_statmap_form(image_path=os.path.join(app_path, 'test_data/afqmaps/all.nii.gz'),
                                           collection=collection1,
                                           image_name="image1")
        image2 = save_statmap_form(image_path=os.path.join(app_path, 'test_data/afqmaps/all.nii.gz'),
                                           collection=collection2,
                                           image_name="image2")

...
def move_collection_data(apps, schema_editor):
    ...
    cursor.execute("""SELECT setval(pg_get_serial_sequence('"afqmaps_basecollectionitem"','id'), coalesce(max("id"), 1), max("id") IS NOT null) FROM "afqmaps_basecollectionitem";""")
    ...
    for nidmresult in NIDMResults.objects.all():
        ...
        new_basebollectionitem.polymorphic_ctype = ContentType.objects.get(app_label="afqmaps", model="nidmresults")

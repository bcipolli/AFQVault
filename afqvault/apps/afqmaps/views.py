...
def view_image(request, pk, collection_cid=None):
    ...
    if isinstance(image, Atlas):
        ...
        if np.isnan(image.perc_bad_voxels) or np.isnan(image.perc_voxels_outside):
            ...
            context[
                'warning'] = "Warning: This map seems to be thresholded, sparse or acquired with limited field of view (%.4g%% of voxels are zeros). " % image.perc_bad_voxels
...
def stats_view(request):
    ...
    non_empty_collections_count = Collection.objects.annotate(
        num_submissions=Count('basecollectionitem')).filter(num_submissions__gt=0).count()
    public_collections_count = Collection.objects.filter(private=False).annotate(
        num_submissions=Count('basecollectionitem')).filter(num_submissions__gt=0).count()
    public_collections_with_DOIs_count = Collection.objects.filter(private=False).exclude(Q(DOI__isnull=True) | Q(
        DOI__exact='')).annotate(num_submissions=Count('basecollectionitem')).filter(num_submissions__gt=0).count()
...
def compare_images(request, pk1, pk2):
    ...
    if len(threshold_status) > 0:
        ...
        for i in range(0, len(image_names)):
            warnings.append('Warning: Thresholded image: %s (%.4g%% of voxels are zeros),' %
